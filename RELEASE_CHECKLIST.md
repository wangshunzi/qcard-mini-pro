...
- 虚拟支付分别用咔豆包和固定时长 VIP 覆盖：正常支付、主动取消、弱网、支付成功后杀进程、
  回调晚于客户端返回、重复回调、补单到账、履约失败后重试和退款。客户端只能在
  `paymentStatus=SUCCEEDED` 且 `fulfillmentStatus=SUCCEEDED` 时提示到账。
- 支付页连续点击只能创建一次请求；`wx.login` 必须在每次 prepare 前重新调用，
  本地 pending 记录只能包含订单号、商品和客户端幂等 ID，不得保存签名或 `session_key`。
- 购买后首页、个人中心、资源、卡包详情、学习、教师页和 AI 生成页应立即刷新统一咔豆余额、
  VIP 状态与访问权限；杀进程重启和前后台切换后也应通过订单状态恢复。
- 在购买记录页核对支付、权益、退款三条状态轴；支付成功但权益处理中时不得显示“已到账”。
...
- 覆盖至少一台 iOS 微信、一台 Android 微信和可用的 HarmonyOS/Windows 微信环境，
  验证安全区、键盘、Canvas、拖拽、音视频中断与虚拟支付能力检测。
- iOS 真机使用 iOS 15+、微信 8.0.68+、中国大陆 App Store 环境和正式商品；
  沙箱、低版本或不支持的客户端只能提示升级/暂不支持，不得跳转 App/H5 购买。
